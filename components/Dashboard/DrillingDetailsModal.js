...
export default function DrillingDetailsModal({ date, onClose }) {
    ...
    const grandTotal = data.reduce((acc, row) => ({
        Holes: acc.Holes + (Number(row.NoofHoles) || 0),
        Meters: acc.Meters + (Number(row.TotalMeters) || 0),
        AvgSpacing: acc.AvgSpacing + (Number(row.Spacing) || 0), // Just summing for avg calc later if needed, or just display avg of avgs? Usually totals are sums. 
        // For Spacing/Burden/Depth, a grand total sum doesn't make sense. Maybe weighted average?
        // The image shows averages for Spacing, Burden, Depth in the total row.
        // Let's calculate sums for holes/meters and weighted averages for others if possible, or simple averages.
        // Actually, for Spacing/Burden/Depth in total rows, likely it's an average.
        Count: acc.Count + 1
    }), { Holes: 0, Meters: 0, AvgSpacing: 0, Count: 0 });
    ...
    return (
        <div style={{
            ...
            <div style={{
                background: 'white', borderRadius: '8px', width: '90%', maxWidth: '1200px', maxHeight: '90vh',
                ...
                <div style={{ padding: '1rem', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button onClick={onClose} style={{ padding: '5px', borderRadius: '50%', background: '#f1f5f9', cursor: 'pointer' }}>
                        <X size={20} />
                    </button>
                </div>
