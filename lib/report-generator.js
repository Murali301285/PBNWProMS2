...
export async function generateReportBackground(requestId, reportType, fromDate, toDate, dbName) {
    try {
        ...
        if (reportType === 'MaterialLoading') {
            ...
            ORDER BY T0.LoadingDate ASC, T1.ShiftName ASC
            `;
            ...
            ORDER BY T0.RehandlingDate ASC, T1.ShiftName ASC
            `;
            ...
            SELECT * FROM MainData ORDER BY MainDate ASC, Shift ASC
            `;
            ...
            WITH MainData AS (
                select T0.SlNo,T0.Date as MainDate, FORMAT(T0.Date,'yyyy') as Year, FORMAT(T0.Date,'MMMM-yy') as Month,FORMAT(T0.Date,'dd - MMM - yyyy') as Date,
            ...
            FROM MainData
            ORDER BY MainDate ASC, ShiftName ASC;
            `;
        }
