CREATE OR ALTER PROCEDURE [dbo].[PMS2_New_Dash_SP_Performance_LoadingPerformance]
    @FromDate DATE,
    @ToDate DATE,
...
BEGIN
    ...
    BEGIN
        ...
        SELECT 
            ...
            CASE WHEN MAX(ER.TotalWorkingHr) > 0 THEN SUM(L.TotalQty) / MAX(ER.TotalWorkingHr) ELSE 0 END AS Rate
        ...
        SELECT 
            ...
            E.Capacity,
            S.ShiftName AS Shift,
            SUM(L.NoofTrip) AS Qty, -- Trip count for Hauling rate? Frontend says Trip/Hr.
            MAX(ER.TotalWorkingHr) AS Hrs,
            CASE WHEN MAX(ER.TotalWorkingHr) > 0 THEN SUM(L.NoofTrip) / MAX(ER.TotalWorkingHr) ELSE 0 END AS Rate
        FROM Trans.TblLoading L WITH(NOLOCK)
        JOIN Trans.TblEquipmentReading ER WITH(NOLOCK) 
            ON L.HaulerEquipmentId = ER.EquipmentId 
            AND L.LoadingDate = ER.Date 
            AND L.ShiftId = ER.ShiftId
            AND ER.ActivityId = 4 -- Hauling
