...
-- 3. Production Data (Coal & OB from TblLoading)
    ProductionData AS (
        SELECT 
            ...
            -- Coal 
              SUM(CASE WHEN Mt.MaterialName IN ('ROM COAL') AND (ES.Sect
orId IS NULL OR ES.SectorId <> 5) THEN L.TotalQty ELSE 0 END) AS Coal_MainPit,
		      SUM(CASE WHEN Mt.MaterialName IN ('ROM COAL') AND ISNULL(ES.SectorId, 0) = 5 THEN L.TotalQty ELSE 0 END) AS Coal_WP3,

            -- OB
            SUM(CASE WHEN Mt.Ma
terialName IN ('OB', 'OVER BURDEN') AND (ES.SectorId IS NULL OR ES.SectorId <> 5) THEN L.TotalQty ELSE 0 END) AS OB_MainPit,
            SUM(CASE WHEN Mt.MaterialName IN ('OB', 'OVER BURDEN') AND ISNULL(ES.SectorId, 0) = 5 THEN L.TotalQty ELSE 0 END) AS O
B_WP3
        ...
        LEFT JOIN 
Master.TblMaterial Mt WITH(NOLOCK) 
    ...
    -- 4. Rehandling Data 
(Coal & OB from TblMaterialRehandling)
    RehandlingData AS (
        SELECT 
            CAST(RH.RehandlingDate AS DATE) AS RehandlingDate,

            -- Coal Rehandling
            SUM(CASE WHEN Mt.MaterialName IN ('ROM COAL') AND (ES.SectorId IS NUL
L OR ES.SectorId <> 5) THEN RH.TotalQty ELSE 0 END) AS CoalRehandling_MainPit,
            SUM(CASE WHEN Mt.MaterialName IN ('ROM COAL') AND ISNULL(ES.SectorId, 0) = 5 THEN RH.TotalQty ELSE 0 END) AS CoalRehandling_WP3,

            -- OB Rehandling
     
       SUM(CASE WHEN Mt.MaterialName IN ('OB', 'OVER BURDEN')  AND (ES.SectorId IS NULL OR ES.SectorId <> 5) THEN RH.TotalQty ELSE 0 END) AS OBRehandling_MainPit,
            SUM(CASE WHEN Mt.MaterialName IN ('OB', 'OVER BURDEN') AND ISNULL(ES.SectorId, 0
) = 5 THEN RH.TotalQty ELSE 0 END) AS OBRehandling_WP3
 ...
 GROUP BY CAST(RH.RehandlingDate AS DATE)
    ...
    LEFT JOIN RehandlingData RH O
N D.DateValue = RH.RehandlingDate
