...
async function renameSp() {
    try {
        ...
        try {
            await mssql.query("DROP PROCEDURE [dbo].[PMS2_New_Sp_OperatorPerformanceHaulingReport]");
            console.log("Dropped existing target SP if any.");
        } catch (e) {
            // ignore if not exists
        }

        await mssql.query("exec sp_rename 'PMS2_New_Sp_OperatorPerformanceReport_Hauling', 'PMS2_New_Sp_OperatorPerformanceHaulingReport'");
