...
BEGIN
    ...
    -- =============================================
    -- 2. Get Aggregated Equipment Readings
    -- =============================================
    EquipmentReadingData AS (
        SELECT 
            R.EquipmentId,
            R.SectorId,
            R.PatchId,
            R.MethodId,
            SUM(ISNULL(R.TotalWorkingHr, 0)) AS TotalWorkingHr
        FROM [Trans].[TblEquipmentReading] R WITH(NOLOCK)
        WHERE R.IsDelete = 0
          AND CAST(R.Date AS DATE) = @Date
          AND (@ShiftId IS NULL OR R.ShiftId = @ShiftId)
        GROUP BY 
            R.EquipmentId, R.SectorId, R.PatchId, R.MethodId
    )
    SELECT
        S.SectorName,
        E.EquipmentName,
