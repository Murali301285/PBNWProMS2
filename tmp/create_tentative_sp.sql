...
CREATE OR ALTER PROCEDURE [dbo].[PMS2_New_Sp_TentativeProductionQty]
    @Date DATE,
    @ShiftId INT
AS
BEGIN
    ...
    WITH WP3Table As( select  T1.EquipmentGroupId,T2.Name as EquipmentGroup,T3.Name as Scale,T0.MaterialId,SUM(T0.NoofTrip) AS NoofTrip,T0.QtyTrip
        ...
        Join Master.TblEquipmentGroup T2 WITH(NOLOCK) on T2.SlNo=T1.EquipmentGroupId
        Join Master.TblScale T3 WITH(NOLOCK) on T3.SlNo=T1.ScaleId
        Where T0.MaterialId in (@TopSoilId,@OBId) and T0.DestinationId=@DestinationDumpB and CAST(T0.LoadingDate as Date)=@Date and T0.ShiftId=@ShiftId
        group by T1.EquipmentGroupId,T2.Name,T3.Name,T0.MaterialId,T0.QtyTrip
    )
    select T0.EquipmentGroupId,T0.EquipmentGroup,T0.Scale,
    ISNULL(T1.OverBurden,0) as OverBurden,ISNULL(T1.OverBurdenFactor,0) as OverBurdenFactor,
