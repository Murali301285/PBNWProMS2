const fs = require('fs');
const file = 'app/dashboard/reports/breakdown-time-analysis/page.js';
...
// Extract existing regex replacement carefully
const oldExportStart = code.indexOf('const handleExportExcel = () => {');
const oldExportEnd = code.indexOf('return (', oldExportStart);

if (oldExportStart !== -1 && oldExportEnd !== -1) {
    const oldExportLogic = code.substring(oldExportStart, oldExportEnd);
    code = code.replace(oldExportLogic, exportLogic + "\n\n    ");
}

fs.writeFileSync(file, code);
console.log("Injected Breakdown Time Analysis Custom Excel Logic!");
