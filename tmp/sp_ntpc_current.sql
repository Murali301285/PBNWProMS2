CREATE PROCEDURE [dbo].[PMS2_New_Sp_ProductionNTPCReport]
    @Date DATE = NULL,
    @ShiftId INT = NULL
AS
BEGIN
    ...
    DECLARE @DestinationDumpB INT = 2; -- Assumed to be WP-3 OB Dump (Kept for reference, though logic changed)
    ...
    -- =============================================
    -- 2. WP-3 Quantity (Modified: SectorId = 5 from EquipmentReading)
    -- =============================================
    WITH WP3Table AS (
        SELECT
            T1.EquipmentGroupId,
        ...
        WHERE T0.IsDelete = 0
          ...
          AND CONVERT(DATE, T0.LoadingDate) = @Date
          AND T0.ShiftId = @ShiftId
          -- CHANGED LOGIC: Use Equipment from Sector 5
    ...
    WITH FilterDate AS (
        ...
        WHERE T0.IsDelete = 0
          AND T0.MaterialId IN (@OBId, @TopSoilId, @RomCoalId)
          AND T0.DestinationId = @DestinationDumpB -- Note: Header info still uses DestinationId? User didn't ask to change this, but usually header should match report scope.
          -- However, user specifically asked for "wp-3 Quantity" calculation. I'll stick to that to be safe.
          AND CONVERT(DATE, T0.LoadingDate) = @Date
          AND T0.ShiftId = @ShiftId
