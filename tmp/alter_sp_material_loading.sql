CREATE OR ALTER PROCEDURE [dbo].[PMS2_New_Sp_MaterialLoadingReport]
    @FromDate DATE,
    @ToDate DATE,
...
BEGIN
    ...
    -- Local Variable to easily exclude ActivityId 4
    ...
    SELECT 
        ...
        T14.Name as ActivityName  -- Added Activity Name Output
    ...
    -- Relaxed Join for Sector/Patch/Activity
    ...
    LEFT JOIN [Master].TblActivity T14 WITH(NOLOCK) ON T14.SlNo=T11.ActivityId  -- Join Activity Master

    WHERE T0.IsDelete = 0 
    AND T0.ActivityId <> @ActivityId  -- Exclude Hauling Data (ActivityId = 4)
