...
BEGIN
    ...
    SELECT 
        ...
        -- New Aggregations
        ISNULL(L.CoalTrips, 0) + ISNULL(L.OBTrips, 0) AS [TOTAL TRIPS],
        CAST((ISNULL(L.CoalQty, 0) / 1.55) + ISNULL(L.OBQty, 0) AS DECIMAL(18,2)) AS [TOTAL QTY],
        T0.NetHMR AS [TOTAL HRS],

        CASE WHEN T0.NetHMR > 0 
             THEN CAST(ROUND((ISNULL(L.CoalTrips, 0) + ISNULL(L.OBTrips, 0)) / T0.NetHMR, 0) AS INT) 
             ELSE 0 END AS [TOTAL TRIPS PER HR],
             
        CASE WHEN T0.NetHMR > 0 
             THEN CAST(ROUND(((ISNULL(L.CoalQty, 0) / 1.55) + ISNULL(L.OBQty, 0)) / T0.NetHMR, 0) AS INT) 
             ELSE 0 END AS [TOTAL BCM/HR],
...
fs.writeFileSync('f:/Dev/ProMS/ProMSDev/tmp/update_sp_operator_loading.sql', spDefinition);
...
fs.writeFileSync('f:/Dev/ProMS/ProMSDev/tmp/run_operator_loading_sp.js', sqlCmd);
