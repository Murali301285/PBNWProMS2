...
BEGIN
    ...
    LEFT JOIN [Master].[TblRelay] R WITH(NOLOCK) ON T0.RelayId = R.SlNo -- Added Relay Join
    LEFT JOIN [Master].[TblEquipment] Eq WITH(NOLOCK) ON T0.EquipmentId = Eq.SlNo
    
    -- Outer Apply for Loading Data
    OUTER APPLY (
        SELECT 
    ...
    WHERE T0.IsDelete = 0
      ...
      -- Exclude Haulers (ActivityId 4)
      AND (Eq.ActivityId != @ActivityId AND Eq.EquipmentName NOT LIKE '%TIPPER%' AND Eq.EquipmentName NOT LIKE '%DUMPER%')
