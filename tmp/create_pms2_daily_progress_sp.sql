...
CREATE OR ALTER PROCEDURE [dbo].[PMS2_New_Sp_DailyProgressReport]
	@Date DATE = NULL
AS
BEGIN
	SET NOCOUNT ON;
    ...
    -- Logic: Coal (MT / 1.55) + Waste (BCM)
    DECLARE @CoalFactor DECIMAL(18,2) = 1.55;

    INSERT INTO #TempProduction (SlNo, MaterialName, Unit, DayTrip, DayQty, MonthTrip, MonthQty, YearTrip, YearQty, OrderNo)
    SELECT 
        0, 'TOTAL EXCAVATION', 'BCM (MT/1.55)',
        SUM(DayTrip), -- Total Trips
        SUM(CASE WHEN Unit = 'MT' THEN DayQty / @CoalFactor ELSE DayQty END), -- Coal/1.55 + BCM
        SUM(MonthTrip),
        SUM(CASE WHEN Unit = 'MT' THEN MonthQty / @CoalFactor ELSE MonthQty END),
        SUM(YearTrip),
        SUM(CASE WHEN Unit = 'MT' THEN YearQty / @CoalFactor ELSE YearQty END),
        7
    FROM #TempProduction 
