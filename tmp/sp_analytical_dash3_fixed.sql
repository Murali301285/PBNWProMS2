...
BEGIN
    ...
    SELECT 'coal_prod', ISNULL(S.SectorName, 'Unknown'), CAST(L.LoadingDate AS DATE), L.TotalQty

    FROM Trans.TblLoading L WITH(NOLOCK)

    LEFT JOIN Trans.TblEquipmentReading ER WITH(NOLOCK) ON 

            CAST(ER.Date AS DATE) = CAST(L.LoadingDate AS DATE) 

            AND ER.ShiftId = L.ShiftId 

            AND ER.EquipmentId = L.LoadingMachineEquipmentId 
    ...
    SELECT 'ob_rem', ISNULL(S.SectorName, 'Unknown'), CAST(L.LoadingDate AS DATE), L.TotalQty
