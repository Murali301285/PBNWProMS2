const fs = require('fs');
const file = 'app/dashboard/reports/hauling-master/page.js';
...
const exportLogic = `
    const handleExportExcel = async (sortedData, visibleCols) => {
        try {
            ...
            const ws = wb.addWorksheet('Hauling Master');

            const maxColSpan = visibleCols.length > 0 ? visibleCols.length : 38;

            // 1. Calculate max widths for dynamic columns
            const maxColWidths = {};
            visibleCols.forEach(col => {
                const textCols = ['Cost Center', 'PMS Code', "Operator's Name", 'Hauler', 'Hauling Model', 'Sector', 'Patch', 'Method', 'Remarks'];
            ...
            setCell(ws.getCell('B4'), "Hauling Master Report", { bold: true, align: 'center', border: false, underline: true, fontSize: 12 });
            ...
            sortedData.forEach((row, rIdx) => {
                ...
                visibleCols.forEach((col, cIdx) => {
                    ...
                    if (col.accessor === 'Sl.No') val = rIdx + 1;
                    if (col.render) {
                        const res = col.render(row, rIdx);
                        if (res !== null && typeof res !== 'object') val = res;
                    }
                    ...
                    if (!isNaN(num) && val !== '' && val !== null && col.accessor !== 'Date' && col.accessor !== 'Year' && col.accessor !== 'Month') {
                        ...
                        // Treat HMRs and KMRs with 2 decimals
                        if (['OHMR', 'CHMR', 'Net HMR', 'OKMR', 'CKMR', 'Net KMR', 'Speed', 'Route Length', 'Lead'].includes(col.accessor)) {
                             nFmt = '0.00';
                             if (val % 1 === 0) nFmt = '0';
                        }
                    }
                    ...
                    if (val !== null && val !== undefined) {
                        if (['Cost Center', 'PMS Code', 'Year', 'Month'].includes(col.accessor)) {
                            nFmt = '0';
                        }
                    }

                    const isLeftAlign = ['Cost Center', 'PMS Code', "Operator's Name", 'Hauler', 'Hauling Model', 'Sector', 'Patch', 'Method', 'Remarks'].includes(col.accessor);
            ...
            saveAs(new Blob([buffer]), \`Hauling_Master_\${(fDate || '').replace(/\\//g, '-')}.xlsx\`);
...
console.log("Injected Hauling Master Custom Excel Logic!");
