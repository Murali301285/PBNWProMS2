...
BEGIN
    ...
    ---------------------------------------------------------------------------
    INSERT INTO #HighestProd (Category, PeriodType, Qty, Shift, Date, Month)
    SELECT TOP 1 'Crushing', 'Shift', SUM(TotalQty), S.ShiftName, CAST(L.LoadingDate AS DATE), DATENAME(MONTH, L.LoadingDate)
    FROM Trans.TblLoading L
    JOIN Master.TblShift S ON L.ShiftId = S.SlNo
    ...
    INSERT INTO #HighestProd (Category, PeriodType, Qty, Shift, Date, Month)
    SELECT TOP 1 'Crushing', 'Day', SUM(TotalQty), NULL, CAST(L.LoadingDate AS DATE), DATENAME(MONTH, L.LoadingDate)
    FROM Trans.TblLoading L
    JOIN Master.TblDestination D ON L.DestinationId = D.SlNo
    WHERE L.IsDelete = 0 
      AND (D.Name LIKE '%Crush%' OR D.Name LIKE '%CHP%')
    ...
    INSERT INTO #HighestProd (Category, PeriodType, Qty, Shift, Date, Month)
    SELECT TOP 1 'Crushing', 'Month', SUM(TotalQty), NULL, NULL, DATENAME(MONTH, L.LoadingDate)
    FROM Trans.TblLoading L
    JOIN Master.TblDestination D ON L.DestinationId = D.SlNo
    WHERE L.IsDelete = 0 
      AND (D.Name LIKE '%Crush%' OR D.Name LIKE '%CHP%')
...
async function run() {
    try {
        ...
        console.log("Updating Dash SP to source Crushing from TblLoading Destinations...");
