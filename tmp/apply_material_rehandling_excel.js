const fs = require('fs');
const file = 'app/dashboard/reports/material-rehandling/page.js';
...
const exportLogic = `
    const handleExportExcel = async (sortedData, visibleCols) => {
        try {
            ...
            const ws = wb.addWorksheet('Material Rehandling');
            ...
            visibleCols.forEach(col => {
                if (['CostCenterLoading', 'ProdsysCodeLoading', 'CostCenterHauler', 'ProdsysCodeHauling', 'SourceName', 'Destination', 'HaulerEquipment', 'LoadingMachine', 'MaterialName', 'LoadingModel', 'HaulingModel', 'ShiftInchargeLarge', 'ShiftInchargeMid', 'Sector', 'Patch', 'ScaleName', 'Remarks'].includes(col.accessor)) {
            ...
            setCell(ws.getCell('B4'), "Material Rehandling Report", { bold: true, align: 'center', border: false, underline: true, fontSize: 12 });
            ...
            sortedData.forEach((row, rIdx) => {
                ...
                visibleCols.forEach((col, cIdx) => {
                    ...
                    if (val !== null && val !== undefined) {
                        if (['CostCenterLoading', 'ProdsysCodeLoading', 'CostCenterHauler', 'ProdsysCodeHauling', 'Year', 'Month'].includes(col.accessor)) {
                            nFmt = '0';
                        }
                    }

                    const isLeftAlign = ['CostCenterLoading', 'ProdsysCodeLoading', 'CostCenterHauler', 'ProdsysCodeHauling', 'SourceName', 'Destination', 'HaulerEquipment', 'LoadingMachine', 'MaterialName', 'LoadingModel', 'HaulingModel', 'ShiftInchargeLarge', 'ShiftInchargeMid', 'ScaleName', 'Remarks'].includes(col.accessor);
            ...
            saveAs(new Blob([buffer]), \`Material_Rehandling_\${(fDate || '').replace(/\\//g, '-')}.xlsx\`);
...
console.log("Injected Material Rehandling Custom Excel Logic!");
