...
BEGIN
            ...
            -- Relaxed Join for Sector/Patch (Removed Strict ActivityId check if causing issues, or rely on Equipment/Date/Shift)
            LEFT JOIN [Trans].TblEquipmentReading T11 WITH(NOLOCK) on 
                ...
                AND T11.IsDelete=0
                -- AND T11.ActivityId=@ActivityId  <-- Removed strict check to ensure we get *any* reading for this machine/shift that defines its location
