...
BEGIN
    ...
    SELECT 
        ...
        -- WP-3 Coal: Material = RomCoal AND Dest = DumpB
        @WPCoalQty = SUM(CASE 
            WHEN MaterialId = @RomCoalId AND DestinationId = @DestinationDumpB THEN (NoofTrip * QtyTrip) 
            ELSE 0 
        END),

        -- WP-3 OB: Material in (TopSoil, OB) AND Dest = DumpB
        @WPObQty = SUM(CASE 
            WHEN MaterialId IN (@TopSoilId, @OBId) AND DestinationId = @DestinationDumpB THEN (NoofTrip * QtyTrip) 
            ELSE 0 
        END),
    ...
    WHERE IsDelete = 0 
      AND Convert(DATE, LoadingDate) = @Date 
      AND ShiftId = @ShiftId;
    ...
    SELECT  
        T1.Name AS Plant,
        ISNULL(SUM(T0.RunningHr), 0) AS RunningHr,
        ISNULL(SUM(T0.TotalQty), 0) AS TotalQty 
