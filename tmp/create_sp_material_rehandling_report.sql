CREATE OR ALTER PROCEDURE [dbo].[PMS2_New_Sp_MaterialRehandlingReport]
    @FromDate DATE,
    @ToDate DATE,
    ...
    @PatchIds NVARCHAR(MAX) = NULL, -- Note: Patch logic might be sparse in Rehandling, but keeping for consistency
    @InchargeIds NVARCHAR(MAX) = NULL
AS
BEGIN
    SET NOCOUNT ON;
    ...
    IF @InchargeIds = '' SET @InchargeIds = NULL;
    ...
    AND (@ScaleIds IS NULL OR T4.ScaleId IN (SELECT value FROM STRING_SPLIT(@ScaleIds, ','))) -- Note: Scale is usually on Equipment
    ...
    AND (@InchargeIds IS NULL OR (
        T0.ShiftInchargeId IN (SELECT value FROM STRING_SPLIT(@InchargeIds, ',')) 
        OR 
        T0.MidScaleInchargeId IN (SELECT value FROM STRING_SPLIT(@InchargeIds, ','))
    ))
