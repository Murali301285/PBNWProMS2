ALTER PROCEDURE [dbo].[PMS2_New_Sp_MaterialLoadingReport]
    @FromDate DATE,
    @ToDate DATE,
    ...
    @ShiftInchargeIds NVARCHAR(MAX) = NULL,    
    @MidScaleInchargeIds NVARCHAR(MAX) = NULL  
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SELECT 
        ROW_NUMBER() OVER(ORDER BY T0.LoadingDate ASC, T1.SlNo ASC) as SlNo,
        FORMAT(T0.LoadingDate,'yyyy') as Year, 
        FORMAT(T0.LoadingDate,'MMMM-yy') as Month,
        -- [MODIFIED] Changed 'dd/MM/yyyy' to 'dd/MMM/yyyy' for 01/Apr/2026 format!
        FORMAT(T0.LoadingDate,'dd-MMM-yyyy') as Date,
        T1.ShiftName,
    ...
    LEFT JOIN [Trans].TblEquipmentReading T11 WITH(NOLOCK) on CONVERT(date,T11.Date)=CONVERT(date,T0.LoadingDate) 
