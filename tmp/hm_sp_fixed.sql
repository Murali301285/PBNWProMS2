ALTER   PROCEDURE [dbo].[PMS2_New_Sp_HaulingMasterReport]
(
    @fromDateInput DATE,
    @toDateInput DATE,
...
BEGIN
    ...
    WITH MainData AS (
        SELECT 
           ...
           -- Op.OperatorName as Operator,
			CONCAT(Op.OperatorName, '-', Op.OperatorId, '') as Operator,
            T1.ShiftName, 
            ...
            -- Logic for distributing hours
             CASE WHEN ISNULL(T8.CoalTrips,0.00)>0 THEN (ISNULL(T0.TotalWorkingHr,0.00)/(ISNULL(T8.CoalTrips,0.00)+ISNULL(T7.OBTRIPS,0.00)+ISNULL(T9.RehandlingOBTRIPS,0.00)+ISNULL(T10.RehandlingCoalTrips,0.00)+ISNULL(T11.RehandlingOtherTrips,0.00))*ISNULL(T8.CoalTrips,0.00)) ELSE 0.00 END as CoalHrs,
             CASE WHEN ISNULL(T7.OBTRIPS,0.00)>0 THEN (ISNULL(T0.TotalWorkingHr,0.00)/(ISNULL(T8.CoalTrips,0.00)+ISNULL(T7.OBTRIPS,0.00)+ISNULL(T9.RehandlingOBTRIPS,0.00)+ISNULL(T10.RehandlingCoalTrips,0.00)+ISNULL(T11.RehandlingOtherTrips,0.00))*ISNULL(T7.OBTRIPS,0.00)) ELSE 0.00 END as OBHrs,
            
             CASE WHEN ISNULL(T10.RehandlingCoalTrips,0.00)>0 THEN (ISNULL(T0.TotalWorkingHr,0.00)/(ISNULL(T8.CoalTrips,0.00)+ISNULL(T7.OBTRIPS,0.00)+ISNULL(T9.RehandlingOBTRIPS,0.00)+ISNULL(T10.RehandlingCoalTrips,0.00)+ISNULL(T11.RehandlingOtherTrips,0.00))*ISNULL(T10.RehandlingCoalTrips,0.00)) ELSE 0.00 END as MainCoalRehandlingHrs,
             CASE WHEN ISNULL(T9.RehandlingOBTRIPS,0.00)>0 THEN (ISNULL(T0.TotalWorkingHr,0.00)/(ISNULL(T8.CoalTrips,0.00)+ISNULL(T7.OBTRIPS,0.00)+ISNULL(T9.RehandlingOBTRIPS,0.00)+ISNULL(T10.RehandlingCoalTrips,0.00)+ISNULL(T11.RehandlingOtherTrips,0.00))*ISNULL(T9.RehandlingOBTRIPS,0.00)) ELSE 0.00 END as MainOBRehandlingHrs,
    ...
    SELECT 
        ...
        Relay, OHMR, CHMR, NetHMR as [Net HMR], 
        
        FORMAT(TotalWorkingHr, 'N2', 'en-IN') as [Total Working Hr],
        FORMAT(CoalHrs, 'N2', 'en-IN') as [Coal Hrs],
        FORMAT(OBHrs, 'N2', 'en-IN') as [OB Hrs],
        FORMAT(MainCoalRehandlingHrs, 'N2', 'en-IN') as [Coal Rehandling Hrs],
        FORMAT(MainOBRehandlingHrs, 'N2', 'en-IN') as [OB Rehandling Hrs],
        
        FORMAT(OBTRIPS, 'N0', 'en-IN') as [OB Trips],
        FORMAT(QuantityBcm, 'N2', 'en-IN') as [Quantity (BCM)],
        
        FORMAT(CoalTrips, 'N0', 'en-IN') as [Coal Trips],
        FORMAT(QuantityMt, 'N2', 'en-IN') as [Quantity (MT)],
        ...
        END, 'N2', 'en-IN') AS [BCM/Hrs],

        FORMAT(RehandlingCoalTrips, 'N0', 'en-IN') as [Coal Rehandling Trips],
        FORMAT(RehandlingCoalQty, 'N2', 'en-IN') as [Coal Rehandling Qty],
        FORMAT(RehandlingOBTRIPS, 'N0', 'en-IN') as [OB Rehandling Trips],
        FORMAT(RehandlingOBQty, 'N2', 'en-IN') as [OB Rehandling Qty],
        FORMAT(RehandlingOtherTrips, 'N0', 'en-IN') as [Other Rehandling Trips],
        FORMAT(RehandlingOtherQty, 'N2', 'en-IN') as [Other Rehandling Qty],
