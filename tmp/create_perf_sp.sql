...
BEGIN
    ...
    -- Electrical (EquipmentGroup Name like '%Elect%')
    INSERT INTO #HighestProd (Category, PeriodType, Qty, Shift, Date, Month)
    ...
    -- Dispatch (Assuming TblDispatchEntry)
    INSERT INTO #HighestProd (Category, PeriodType, Qty, Shift, Date, Month)
    ...
    WHERE CAST(C.Date AS DATE) BETWEEN @FromDate AND @ToDate AND C.IsDelete = 0
    GROUP BY P.Name;

    -- [2] Result Set 3: Sector Wise (Using Source as Sector)
    SELECT 
        ISNULL(S.Name, 'Unknown') AS Plant, -- Mapped to 'Plant' accessor
        ISNULL(SUM(L.TotalQty), 0) AS Qty
    FROM Trans.TblLoading L
    LEFT JOIN Master.TblSource S ON L.SourceId = S.SlNo
    WHERE CAST(L.LoadingDate AS DATE) BETWEEN @FromDate AND @ToDate AND L.IsDelete = 0
    GROUP BY S.Name;
    ...
    SELECT * FROM (
        ...
        CROSS APPLY (SELECT SUM(ISNULL(TotalWorkingHr,0)) as TotalHours FROM Trans.TblEquipmentReading ER WHERE ER.EquipmentId = L.LoadingMachineEquipmentId AND CAST(ER.Date AS DATE) = CAST(L.LoadingDate AS DATE)) R
        WHERE CAST(L.LoadingDate AS DATE) BETWEEN @FromDate AND @ToDate AND L.IsDelete = 0
        ...
        LEFT JOIN Master.TblShift S ON L.ShiftId = S.SlNo
        CROSS APPLY (SELECT SUM(ISNULL(TotalWorkingHr,0)) as TotalHours FROM Trans.TblEquipmentReading ER WHERE ER.EquipmentId = L.HaulerEquipmentId AND CAST(ER.Date AS DATE) = CAST(L.LoadingDate AS DATE)) R
        WHERE CAST(L.LoadingDate AS DATE) BETWEEN @FromDate AND @ToDate AND L.IsDelete = 0
        GROUP BY E.EquipmentName, E.Model, E.Capacity, S.ShiftName
    ) AS OpsPerf
    ...
    SELECT 
        ...
        Shift
    FROM (
        -- Inner similar to above but summarized
         SELECT 
            'Loading' AS Type,
            ISNULL(E.EquipmentName, 'Unknown') AS Equipment,
            ISNULL(SUM(L.TotalQty), 0) AS Qty,
