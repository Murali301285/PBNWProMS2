CREATE   PROCEDURE [dbo].[PMS2_New_Sp_HaulingMasterReport]
(
    @fromDateInput DATE,
    @toDateInput DATE,
...
BEGIN
    ...
    -- Parse CSV Parameters into Table Variables for filtering
    ...
    -- Parsing Operator IDs (assuming they are EquipmentIds or independent Operator IDs logic depending on schema)
    -- In TblEquipmentReading, Operator is derived via dbo.GetOperatorName.
    -- However, often OperatorId is stored or linked. Let's check TblEquipmentReading Schema or similar.
    -- Re-using logic from Loading Master: GetOperatorName uses SlNo and 'EquipmentReading'.
    -- But for filtering, we likely need a specific ID column. 
    -- The prompt asked for "Operator" filter. 
    -- In Loading Master, we used T0.OperatorId (if available) or similar.
    -- Let's assume T0.OperatorId exists as per previous task findings.
    ...
    WITH MainData AS (
        SELECT 
            ...
            dbo.GetOperatorName(T0.SlNo,'EquipmentReading') as Operator,
            T1.ShiftName, 
            T2.CostCenter,
