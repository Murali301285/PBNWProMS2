...
CREATE   PROCEDURE [dbo].[PMS2_New_Dash_SP_GetDrillingBlastingStats]
    @Date DATE
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- =============================================
    -- 5. Explosive Summary (Donut) - Pivoted with Null Handling
    -- =============================================
    SELECT 
        CAST(ISNULL(SUM(SMEQty), 0) AS DECIMAL(18,2)) AS TotalSME,
        CAST(ISNULL(SUM(CASE WHEN (TotalExplosiveUsed - SMEQty) > 0 THEN (TotalExplosiveUsed - SMEQty) ELSE 0 END), 0) AS DECIMAL(18,2)) AS TotalANFO,
        0.00 AS TotalLDE,
        CAST(ISNULL(SUM(TotalExplosiveUsed), 0) AS DECIMAL(18,2)) AS GrandTotal
