                ...
                <div className={`${styles.actions} items-center`}>
                    {lastEntry && (
                        <span style={{
                            ...
                            Last data entered on -&gt; Date: {new Date(lastEntry.CreatedDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).replace(/ /g, '-')} | RehandlingDate :{new Date(lastEntry.Date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).replace(/ /g, '-')} | Entered by : {lastEntry.CreatedByName || lastEntry.CreatedBy || 'Admin'}
                        </span>
                    )}
                    <button
